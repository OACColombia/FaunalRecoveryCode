...
sort(outlyers$out) # >1.589
data2<-subset(data, data$RRS2<1.589)
boxplot(data2$RRS2)
...
m274b<-lm(RRS2~Forest.cov+Continent_Island+Biogeographic_realms+Nbiome+Previous_Use, data=data2)
summary(m274b)
Am274b<-anova(m274b)
Am274bss<-Am274b$"Sum Sq"
print(cbind(Am274b,PctExp=Am274bss/sum(Am274bss)*100)) #PctExp: % explain variation

m4b<-lm(RRS2~Continent_Island, data=data2)
summary(m4b)
Am4b<-anova(m4b)
Am4bss<-Am4b$"Sum Sq"
print(cbind(Am4b,PctExp=Am4bss/sum(Am4bss)*100)) #PctExp: % explain variation

m291b<-lm(RRS2~Forest.cov+Biogeographic_realms+Nbiome+Mean_Elevation+Previous_Use, data=data2)
summary(m291b)
Am291b<-anova(m291b)
Am291bss<-Am291b$"Sum Sq"
print(cbind(Am291b,PctExp=Am291bss/sum(Am291bss)*100)) #PctExp: % explain variation

m271b<-lm(RRS2~Forest.cov+Continent_Island+Biogeographic_realms+Nbiome+Mean_Elevation, data=data2)
summary(m271b)
Am271b<-anova(m271b)
Am271bss<-Am271b$"Sum Sq"
print(cbind(Am271b,PctExp=Am271bss/sum(Am271bss)*100)) #PctExp: % explain variation
