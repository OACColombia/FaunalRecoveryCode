...
## Saving data.frames
write.table(df_top_ranked, "Top_rankedMam.txt",quote=F)
write.table(df_null_model_AICc, "Null_model_AICcMam.txt",quote=F)
write.table(df_null_model_position, "Null_model_positionMam.txt",quote=F)
...
## Saving data.frames
write.table(plausible_mod_frequency, "Plausible_mod_frequencyMam.txt",quote=F, sep="\t")
write.table(plausible_mod_table_AICc, "Plausible_mod_table_AICc_meanMam.txt",quote=F, sep="\t")
write.table(plausible_Null_table_AICc, "Plausible_Null_table_AICc_meanMam.txt",quote=F, sep="\t")
...
#8  ( 516) <- k = 3
#1  ( 467) <- k = 2
m5b<-lm(RRS2~Biogeographic_realms, data=data2)
summary(m5b)
Am5b<-anova(m5b)
Am5bss<-Am5b$"Sum Sq"
print(cbind(Am5b,PctExp=Am5bss/sum(Am5bss)*100)) #PctExp: % explain variation

m34b<-lm(RRS2~Biogeographic_realms+Mean_Rainfall, data=data2)
summary(m34b)
Am34b<-anova(m34b)
Am34bss<-Am34b$"Sum Sq"
print(cbind(Am34b,PctExp=Am34bss/sum(Am34bss)*100)) #PctExp: % explain variation
...
m8b<-lm(RRS2~Mean_Rainfall, data=data2)
summary(m8b)
Am8b<-anova(m8b)
Am8bss<-Am8b$"Sum Sq"
print(cbind(Am8b,PctExp=Am8bss/sum(Am8bss)*100)) #PctExp: % explain variation
