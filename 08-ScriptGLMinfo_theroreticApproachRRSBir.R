...
sort(outlyers$out) # >1.465
data2<-subset(data, data$RRS2<1.465)
boxplot(data2$RRS2)
...
## Saving data.frames
write.table(df_top_ranked, "Top_rankedBir.txt",quote=F)
write.table(df_null_model_AICc, "Null_model_AICcBir.txt",quote=F)
write.table(df_null_model_position, "Null_model_positionBir.txt",quote=F)
...
## Saving data.frames
write.table(plausible_mod_frequency, "Plausible_mod_frequencyBir.txt",quote=F, sep="\t")
write.table(plausible_mod_table_AICc, "Plausible_mod_table_AICc_meanBir.txt",quote=F, sep="\t")
write.table(plausible_Null_table_AICc, "Plausible_Null_table_AICc_meanBir.txt",quote=F, sep="\t")
...
m22b<-lm(RRS2~Forest.cov+Mean_Elevation, data=data2)
summary(m22b)
Am22b<-anova(m22b)
Am22bss<-Am22b$"Sum Sq"
print(cbind(Am22b,PctExp=Am22bss/sum(Am22bss)*100)) #PctExp: % explain variation

m23b<-lm(RRS2~Forest.cov+Mean_Rainfall, data=data2)
summary(m23b)
Am23b<-anova(m23b)
Am23bss<-Am23b$"Sum Sq"
print(cbind(Am23b,PctExp=Am23bss/sum(Am23bss)*100)) #PctExp: % explain variation

m15b<-lm(RRS2~Age+Mean_Elevation, data=data2)
summary(m15b)
Am15b<-anova(m15b)
Am15bss<-Am15b$"Sum Sq"
print(cbind(Am15b,PctExp=Am15bss/sum(Am15bss)*100)) #PctExp: % explain variation
