...
names(data)
newdata<-data[c(6,28,53,55,57,59,61,63,65)]#working only with successional stage, Taxa and the proportion of each functional group
head(newdata)
...
ggplot(fungui, aes(x=Guild, y=Prop, alpha=Suc.Stage))+
  ...
  geom_text(data=fgb, aes(x=3.7215, y=0.35, label="*"), size=15,
            show.legend = F)+
  ...
  scale_alpha_discrete(range=c(0, 0.75), name="", labels=c("Early succession", 
                                                           "Young secondary forest",
                                                           "Mid-successional secondary forest",
                                                           "Reference"))+
  ...
  theme(legend.text = element_text(colour=rgb(0,0,0), size = 15),
