...
##changing the graph
#1~ points
p<-ggplot(data,aes(Age,Sim_S_toOGF, 
                    shape=Forest.cov, fill=Forest.cov))+
  geom_point(data=A, aes(A$Age, A$Sim_S_toOGF), 
             size = 5, stroke = 1.1, alpha=0.8, colour="black")+
  geom_point(data=R, aes(R$Age, R$Sim_S_toOGF), 
             size = 5, stroke = 1.1, alpha=0.8, colour="black")+
  geom_point(data=B, aes(B$Age, B$Sim_S_toOGF), 
             size = 5, stroke = 1.1, alpha=0.8, colour="black")+
  geom_point(data=M, aes(M$Age, M$Sim_S_toOGF), 
             size = 5, stroke = 1.1, alpha=0.8, colour="black")
p
#2~some aesthetics
paes<-p+
  ...
  scale_shape_manual(values=c(21,23,24,22,25,8))+
 ...
 scale_fill_manual(values = c("gray100", "gray60", "gray40","gray80"))+
...
lpi<-paes+
  ...
  geom_segment(data=A, aes(x=0, xend=110, y=(SSA$coefficients[1]+SSA$coefficients[2]),
               ...
               colour="black", size=1.2,linetype="solid")+
  geom_segment(data=A, aes(x=0, xend=229, y=(lwPIA$coefficients[1]+lwPIA$coefficients[2]),
               ...
               colour="gray", size=1,linetype="solid")+
...
rTime<-lpi+ 
  geom_segment(data=A, aes(x=4, xend=229, y=1.3, yend=1.3), 
               linetype="solid", colour="gray", size=2)+ 
